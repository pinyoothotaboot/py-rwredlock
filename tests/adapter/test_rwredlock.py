...
class TestRWRedlock(unittest.TestCase):
    ...
    def test_reader_lock_with_data_passed(self):
        read_lock = self._rwredlock.lock(self._lock_id,self._rwredlock.READ,self._ttl,RWLOCK_TIMEOUT)
        locked = self._rwredlock.locked(self._lock_id)
        self._rwredlock.unlock(self._lock_id,self._rwredlock.READ,RWLOCK_TIMEOUT)
    ...
    def test_reader_unlock_with_data_passed(self):
        read_lock = self._rwredlock.lock(self._lock_id,self._rwredlock.READ,self._ttl,RWLOCK_TIMEOUT)
        locked = self._rwredlock.locked(self._lock_id)
        read_unlock = self._rwredlock.unlock(self._lock_id,self._rwredlock.READ,RWLOCK_TIMEOUT)
        self.assertEqual(read_lock,True)
        self.assertEqual(locked,False) # Reader lock already access every
        self.assertEqual(read_unlock,True)
    
    def test_writer_lock_with_data_passed(self):
        write_lock = self._rwredlock.lock(self._lock_id,self._rwredlock.WRITE,self._ttl,RWLOCK_TIMEOUT)
        locked = self._rwredlock.locked(self._lock_id)
        self._rwredlock.unlock(self._lock_id,self._rwredlock.WRITE,RWLOCK_TIMEOUT)
    ...
    def test_writer_unlock_with_data_passed(self):
        ...
        write_unlock = self._rwredlock.unlock(self._lock_id,self._rwredlock.WRITE,RWLOCK_TIMEOUT)

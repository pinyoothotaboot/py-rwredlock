...
def sumation_number(pid : str):
    ...
    for idx in range(100):
        ...
        if rwredlock.lock(lock_id,rwredlock.READ,identifier,RWLOCK_TTL,RWLOCK_TIMEOUT):
            resp = broker.get(lock_key)
        rwredlock.unlock(lock_id,rwredlock.READ,identifier,RWLOCK_TIMEOUT)

        if not resp:
            if rwredlock.lock(lock_id,rwredlock.WRITE,identifier,RWLOCK_TTL,RWLOCK_TIMEOUT):
...
if __name__ == "__main__":
    try:
        ...
        for idx in range(cpu):
            processing = multiprocessing.Process(target=sumation_number,args=(f"Worker-{idx}",))
            processing.start()

...
class RWRedlock(LockInterface):
    ...
    def lock(self, lock_id: str,indentifier : str, ttl: int = LOCK_TTL, lock_timeout: int = LOCK_TIMEOUT) -> bool:
    ...
    def unlock(self, lock_id: str,indentifier : str,ttl : int, lock_timeout: int) -> bool:
        lock_timeout: int = lock_timeout if lock_timeout > 0 else LOCK_TIMEOUT
        ttl: int = ttl if ttl > 0 else LOCK_TTL
        end = get_time(ttl)
        lock_id = self.__get__lock_id(lock_id)
